...
plot(Q8,main="Quadrat Count Plot")
...
# regular expression to find rows where Naltrexone is used in treatment
...
# mark cases where both Buprenorphine and negative contexts are mentioned
...
segregation.test(Y.m, nsim = 20, hmin = 0.05, hmax = 5)
...
coords_df_y <- data.frame(
  ...
  Facility_Type = factor(Y.m$marks, levels = c("No Medication", "Naltrexone", "Buprenorphine","Naltrexone and Buprenorphine"))
)

#subset data by treatment type

# Subset the data by treatment type
coords_df_naltrexone <- coords_df_y[coords_df_y$Facility_Type == "Naltrexone", ]
coords_df_buprenorphine <- coords_df_y[coords_df_y$Facility_Type == "Buprenorphine", ]
coords_df_no_medication <- coords_df_y[coords_df_y$Facility_Type == "No Medication", ]
coord_df_both <-coords_df_y[coords_df_y$]
...
coords_df_y <- data.frame(
  Longitude = Y.m$x,  # Longitude values from Y.m
  Latitude = Y.m$y,   # Latitude values from Y.m
  Treatment_Type = factor(Y.m$marks, levels = c("No Medication", "Naltrexone", "Buprenorphine","Naltrexone and Buprenorphine"))
)
...
plot(probY)
#consider increasing to 100 or 200 for semester project
segregation.test(Y.m,nsim=200,hmin=0.01,hmax=10)
...
coords_df_y <- data.frame(
  Longitude = Y.m$x,  # Longitude values from X.m2
  Latitude = Y.m$y,   # Latitude values from X.m2
...
colo_map_background +
  ...
  theme_minimal() +
  labs(title = "Mental Health Facility Distribution by Treatment Type",
       subtitle = "Colored by Treatment Type (Naltrexone, Buprenorphine, No Medication, Both)",
       x = "Longitude", y = "Latitude")
